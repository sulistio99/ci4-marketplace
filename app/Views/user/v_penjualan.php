            ...
            <tbody>
              <?php foreach ($penjualan as $key => $value) { ?>
                ...
                <tr>
                  ...
                  <td><?= number_to_currency($value['harga_tawar_pembeli'], 'Rp.') ?></td>
                  <td><span class="text-primary"><?= $status['status']; ?></span></td>
                  <td width="180px">
                    <div class="btn-group">
                      <a href="<?= base_url('User/NegoPenjual/' . $value['id_pengajuan']); ?>" class="btn btn-warning btn-sm"><i class="fas fa-bars"></i> Rincian</a>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">Pembatalan Pengajuan</h4>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <?php echo form_open_multipart(base_url('User/DeletePengajuanPenjualan/' . $value['id_pengajuan'])) ?>
          <input type="hidden" name="id_barang" value="<?= $value['id_barang']; ?>">
          <input type="hidden" name="id_cart" value="<?= $value['id_cart']; ?>">
          <div class="form-group">
            Yakin Mau Batalkan <b><?= $value['nama_barang']; ?></b>
          </div>
        </div>
