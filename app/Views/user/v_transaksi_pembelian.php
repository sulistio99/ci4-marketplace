              ...
              <?php foreach ($transaksi as $key => $value) { ?>
                ...
                <tr>
                  ...
                  <td class="text-center">
                    ...
                    <?php if ($status['status'] == 'Bukti Pengiriman Diterima (Barang Dikirim Kelokasi Pembeli)') { ?>
                      <?= $status['status']; ?> <br>
                      <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modal-barangditerima<?= $value['id_transaksi'] ?>" <?= $status['status'] == 'Barang Diterima' ? 'disabled' : ''; ?>>
                        <i class="fas fa-box"></i> Barang Dterima
                      </button>
                    <?php } else { ?>

                      <?php if ($status['status'] != 'Menunggu Konfirmasi Transaksi Dari Admin.' && $status['status'] != 'Transaksi Disetujui (lanjutkan ke menu transaksi)' && $status['status'] != 'Bukti Pembayaran Ditolak (Pastikan Bukti Pembayaran Asli)') { ?>
                        <span class="text-primary"><?= $status['status']; ?></span>
                      <?php } else { ?>
                        <span class="text-primary"><?= $status['status']; ?></span><br>
                        <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modal-uploadbuktipembayaran<?= $value['id_transaksi'] ?>">
                  ...
                  <td width="180px" class="text-center">
                    <div class="btn-group">
                      <a href="<?= base_url('User/RincianPembelian/' . $value['id_transaksi']); ?>" class="btn btn-warning btn-sm">
                      ...
                      <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#modal-deletetransaksipembelian<?= $value['id_transaksi'] ?>" <?php if ($status['status'] == 'Barang Diterima') { ?> disabled <?php } ?>>
...
<!-- modal Hapus -->
<?php foreach ($transaksi as $key => $value) { ?>
  <div class="modal fade" id="modal-deletetransaksipembelian<?= $value['id_transaksi'] ?>">
...
<!-- modal upload -->
<?php foreach ($transaksi as $key => $value) { ?>
  <div class="modal fade" id="modal-uploadbuktipembayaran<?= $value['id_transaksi'] ?>">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">Upload Bukti Transaksi</h4>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <?php echo form_open_multipart(base_url('User/UploadBuktiTransaksi/' . $value['id_transaksi'])) ?>
          <input type="hidden" name="id_barang" value="<?= $value['id_barang']; ?>">
          <input type="hidden" name="id_cart" value="<?= $value['id_cart']; ?>">
          <input type="hidden" name="id_pengajuan" value="<?= $value['id_pengajuan']; ?>">
          <div class="form-group">
...
<?php foreach ($transaksi as $key => $value) { ?>
  <div class="modal fade" id="modal-barangditerima<?= $value['id_transaksi'] ?>">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">Konfirmasi Barang Diterima</h4>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          ...
          <?php
          $db = \Config\Database::connect();
          $id_bukti_transaksi = $db->table('tb_bukti_transaksi')
          ...
          $id_bukti_pengiriman = $db->table('tb_bukti_pengiriman')
